...
### `runtime-react`
- [x] In addition to `HieroIcon`/`useIconState`, direct runtime-json hooks/components exist for payload-driven playback. (`lib/runtime-react/icon.tsx`, `lib/runtime-react/use-icon.ts`)
...
- [x] Build the runtime React surfaces on top of the runtime store. (`lib/runtime-react/HieroIcon.tsx`, `lib/runtime-react/icon.tsx`, `lib/runtime-react/useIconState.ts`, `lib/runtime-react/use-icon.ts`)
- [x] Wire `useSyncExternalStore` subscriptions and prop-driven state changes. (`lib/runtime-react/HieroIcon.tsx`, `lib/runtime-react/use-icon.ts`, `IconDriver.subscribe()` in `lib/runtime-dom/driver.ts`)
