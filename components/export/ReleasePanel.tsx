 ...
 *
 * Polls the hiero dev server's /api/status endpoint to discover release
 * targets configured in the consumer repo's hiero.config.ts. Once connected,
 * renders a card for each target:
 *
 *   local-directory  →  "Push Source" (sends to dev server; user runs `hiero build`)
 *   git-pr           →  delegates to SyncPrPanel
 *   npm-registry     →  "Push Source" (sends to dev server; user runs `hiero publish`)
 *
 * When the dev server is unreachable a placeholder guides the user to run
 * `hiero dev` in the consumer repo.
 *
    ...
    <div className="grid gap-3">
      ...
      {connectionStatus === 'connected' && releaseTargets.length > 0 ? (
        ...
        <p className="text-[10px] text-muted-foreground text-center py-2">
          No release targets in <code className="font-mono">hiero.config.ts</code>.
        </p>
      ) : (
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      if (result.kind === 'success') {
        ...
        toast({
          title: 'Source pushed',
          description: `Run \`hiero build\` in the consumer repo to write snapshot to ${target.outputDir}`,
        });
      } else {
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      if (result.kind === 'success') {
        toast({
          title: 'Source pushed',
          description: `Run \`hiero publish\` in the consumer repo to release ${target.packageName}`,
        });
      } else {
        toast({ title: 'Push failed', description: result.error, variant: 'destructive' });
      }
...
function ReleasePlaceholder({ devServerUrl }: { devServerUrl: string }) {
  return (
    <div className="rounded-lg border border-dashed border-border/60 px-4 py-5 text-center">
      <p className="text-xs text-muted-foreground">
        Run <code className="font-mono">hiero dev</code> in the consumer repo to connect.
      </p>
      <p className="mt-1 text-[10px] text-muted-foreground font-mono">{devServerUrl}</p>
      <p className="mt-2 text-[10px] text-muted-foreground">
        Release targets from <code className="font-mono">hiero.config.ts</code> will appear here.
      </p>
    </div>
  );
}
