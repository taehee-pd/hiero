...
Use the local `Export to Cuneiform` Figma plugin when you want to move selected icons from Figma into Cuneiform without managing API tokens.

## Install The Plugin

1. Open Figma desktop.
2. Go to `Plugins` → `Development` → `Import plugin from manifest...`.
3. Choose [manifest.json](/Users/taehee/Cuneiform/figma-plugin/export-to-cuneiform/manifest.json).

## Export From Figma

1. Open the Figma file that contains your icon nodes.
2. Select one or more vector or component nodes.
3. Run `Export to Cuneiform`.
...
## Import Into Cuneiform

1. Open `Import icon` in Cuneiform.
2. Choose `Cuneiform Plugin`.
3. Paste the JSON payload or upload the downloaded `.json` file.
4. Click `Import Plugin Payload`.

Cuneiform sanitizes and normalizes each SVG before inserting it into the workspace, so the plugin path uses the same import hardening as the other SVG sources.
