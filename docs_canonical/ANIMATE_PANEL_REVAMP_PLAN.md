...
**Owner:** taehee-pd
**Scope:** (1) Eliminate morphing-strategy confusion and warnings from the Animate panel; (2) Reorganize the panel around the SF Symbols 7 layout patterns from [WWDC 2025 session 337](https://developer.apple.com/videos/play/wwdc2025/337/); (3) Prepare `@hiero/cli` for its first npm publish.
...
- Add a single dev-only badge behind `process.env.NEXT_PUBLIC_HIERO_DEBUG === '1'`
...
| WWDC mode       | Hiero stagger mode        | Default duration offset |
|-----------------|-----------------------------|-------------------------|
...
## 3. npm Deploy Preparation (`@hiero/cli`)

`packages/hiero-cli/package.json` ships a `@hiero/cli` stub at `0.1.0` but has
...
- [ ] **Rename check.** The package is named `@hiero/cli` but lives under
  `packages/hiero-cli/`. Decide on final scope (`@hiero/cli` or `@hiero/cli`)
  before first publish — the name is immutable for the first 72h on npm.
- [ ] **Reserve the scope.** Verify the `@hiero` org exists on npmjs.com (or
...
jobs:
  publish:
    ...
    steps:
      ...
      - run: cd packages/hiero-cli && bun install --frozen-lockfile
      - run: cd packages/hiero-cli && bun run build
      - run: cd packages/hiero-cli && npm publish --provenance --access public
        env:
...
- First publish: bump to `0.2.0` (not `1.0.0`) to signal that the CLI surface is
  still evolving.
- Document in `packages/hiero-cli/README.md`:
...
3. A maintainer has run `npm publish --dry-run` locally and eyeballed the tarball
   contents.
4. The `@hiero` npm org has at least two owners to avoid bus-factor-of-one.
...
- Publishing the web app itself to npm or any registry. Hiero remains a
...
1. **Final scope name** — ship as **`@hiero/cli`**. The product is Hiero, so the
   manifest name wins. The package directory stays `packages/hiero-cli/` for now
...
3. **Dev-debug overlay opt-in** — **Retain the current
   `NEXT_PUBLIC_HIERO_DEBUG` env-var gate.** A rebuild to toggle is acceptable
...
- Rewrite literal `ALL-CAPS` strings in the UI to sentence case: "HIERO" →
  "Hiero", "SIZES" → "Sizes", "STATES" → "States", "SAVED EFFECTS" → "Saved
...
| Relationship between sidebar ZIP and the Hiero menu Export submenu is unexplained | Med | Consolidate all export actions under a single "Export" entry point. If both exist, label them unambiguously ("Quick ZIP" vs "Full Export Options"). |
...
- `packages/hiero-cli/package.json` — CLI manifest to finalize
