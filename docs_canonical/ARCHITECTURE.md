...
- `figma-plugin/export-to-hiero/`: Figma plugin for exporting components to Hiero.
...
|------|----------|--------|
| `react-adapter.ts` | React/TypeScript | `HieroIcon`-wrapping `.tsx` components with typed props |
...
- `lib/runtime-react/`: React wrapper components (`HieroIcon` with `forwardRef`), hooks (`useIconState`, `useAnimationProgress`), and imperative handle API
...
Hiero is a web-only application. Platform abstraction is minimal:
...
- `lib/install-config/`: installation configuration for icon packages
- `packages/hiero-cli/`: `@hiero/cli` CLI for CI integration
