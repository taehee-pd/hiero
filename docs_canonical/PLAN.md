# Cuneiform — Production Readiness Plan (archived)
...
plan that drove Cuneiform from Phase 1 through the 2026-04 ship-readiness
...
| G2 | Export UI wiring | **Shipped** — Lottie panel mounted in `components/editor/Toolbar.tsx` via a `Sheet`. Runtime JSON / SVG package / React library / Distribution entries are all in the Cuneiform menu's Export submenu (`components/studio/Navbar.tsx`). |
| G3 | Figma import | **Shipped** — `app/api/import/figma/route.ts` + Figma plugin at `figma-plugin/export-to-cuneiform/` + editor dialog entry point. |
...
| G5 | Desktop distribution | **Removed (R1)** — Cuneiform is now a pure Next.js web application. `desktop/` retains only build artifacts; the Electrobun shell was removed in Phase R1. |
