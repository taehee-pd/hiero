import type { ConivaConfig } from './types';
import { validateConfig } from './validate-config';

export type LoadConfigResult =
  | { ok: true; config: ConivaConfig }
  | { ok: false; error: string };

/**
 * Load and validate a coniva.config.ts module export.
 *
 * In practice, callers will dynamically import coniva.config.ts and pass the
 * default export here. This function validates and normalises the result so
 * consumers always receive a typed, valid ConivaConfig.
 *
 * @param raw - The default export from coniva.config.ts (untyped).
...
export function loadConfig(raw: unknown, configPath?: string): LoadConfigResult {
  ...
  const config = raw as ConivaConfig;
 ...
 */
function normalizeConfig(config: ConivaConfig): ConivaConfig {
  return {
    ...config,
