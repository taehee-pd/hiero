/**
 * Hiero dev server — Lane 1 local HTTP API.
 *
 * Runs in the consumer repo (not in the editor). Consumers install
 * @hiero/cli and run `hiero dev` to start this server.
 *
 * Responsibilities:
...
export async function startDevServer(serverConfig: DevServerConfig): Promise<{
  ...
  console.log(`[hiero] Source directory: ${path.resolve(repoRoot, config.sourceDir)}`);
  ...
  if (initialBuild.kind === 'success') {
    console.log(
      `[hiero] Initial build: ${initialBuild.totalIcons} icon(s) in ${initialBuild.durationMs}ms`,
    );
  ...
  const watcher = watchSourceDir(sourceDir, async () => {
    ...
    if (result.kind === 'success') {
      console.log(
        `[hiero] Rebuilt ${result.changedIcons} icon(s) in ${result.durationMs}ms`,
      );
...
async function handlePostIcons(
  ...
  res: ServerResponse,
  repoRoot: string,
  config: import('@/lib/install-config/types').HieroConfig,
  state: DevServerState,
): Promise<void> {
  ...
  if (result.kind === 'success') {
    console.log(
      `[hiero] API ingest: ${parsed.files.length} file(s) received, rebuilt ${result.changedIcons} icon(s) in ${result.durationMs}ms`,
    );
  }
