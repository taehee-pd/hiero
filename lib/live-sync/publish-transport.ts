 ...
 * consumer repo's hiero dev server.
 *
 * Browser-safe: no Node.js imports. Works in both Next.js and desktop contexts.
 *
 * Transport model:
 *   Editor  ──POST──▶  hiero dev server (localhost:4400)  ──writes──▶  sourceDir
 *
...
/**
 * Publish canonical source files to the consumer repo via the hiero dev
 * server HTTP API.
 *
  ...
  try {
    ...
    if (message.includes('ECONNREFUSED') || message.includes('fetch failed')) {
      return {
        kind: 'error',
        transport: 'api-push',
        error: `Could not connect to hiero dev server at ${baseUrl}. Run \`hiero dev\` in the consumer repo first.`,
      };
    }
...
/**
 * Fetch the full status from the hiero dev server, including release targets.
