...
**Status:** Proposed
**Primary future files:** `lib/live-sync/*`, `packages/hiero-cli`, `lib/sync-service/*`

## Overview

This spec defines how Hiero distributes icon changes in an installable, repo-native model.
...
Hiero should no longer treat `Local / GitHub / npm` as the primary way users experience edits. Those remain valid outputs, but they should happen after Hiero has already updated the host repo or host app locally.

## Core Model

Hiero distribution has two lanes.
...
After ingress, Hiero becomes the system of record.
