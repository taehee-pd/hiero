...
This spec describes the intended user workflow for Hiero as a repo-native icon authoring platform.
...
then Hiero owns the icon system
...
The designer exports icons from Figma using a Hiero plugin.

Expected result:

- the payload lands in Hiero without manual SVG-by-SVG cleanup
- imported icons become canonical source candidates

### Step 2: Hiero becomes SSOT

After import, the user edits icons in Hiero.

Expected result:

- icon structure, variants, transitions, and metadata are owned by Hiero
- Figma is no longer treated as co-equal authority

### Step 3: Repo-aware editing

Hiero is opened against the current codebase context.
...
- back-sync is possible, but it does not move SSOT away from Hiero
...
- the user should understand when Hiero, not Figma, owns the icon set

## Non-Goals

- Hiero should not feel like a generic "send icons to many destinations" panel
...
- icons are inside Hiero
- Hiero is now authoritative
