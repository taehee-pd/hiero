...
Hiero uses a single-screen StudioLayout (similar to Sanity Studio) as the
...
│                                                             │
│  ┌─ Recent Projects ──────────────────────────────────────┐ │
│  │  project-a.hiero  ·  3 days ago  [Load] [×]           │ │
│  │  project-b.hiero  ·  1 week ago  [Load] [×]           │ │
│  └────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
...
> This panel shows the publish status from `hiero.config.ts` and export options.
...
> repo-native distribution configured via `hiero.config.ts` at the repo root.
> The UI shows the current config status and provides publish actions.

### 9A. Host Targets (Live Dev)

```
┌─ Host Targets (from hiero.config.ts) ────────────────────┐
│                                                            │
...
┌─ Config ──────────────────────────────────────────────────┐
│                                                            │
│  Config File   hiero.config.ts  ✓ Found                   │
...
Standalone demo pages that render icons using the Hiero runtime outside of
