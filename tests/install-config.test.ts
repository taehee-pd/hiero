...
import type { ConivaConfig } from '@/lib/install-config';

// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------

const VALID_CONFIG: ConivaConfig = {
  sourceDir: 'coniva',
  hostTargets: [
    {
      kind: 'react-app',
...
describe('loadConfig', () => {
  test('returns ok config for valid input', () => {
    ...
    if (result.ok) {
      expect(result.config.sourceDir).toBe('coniva');
  ...
  test('strips trailing slash from sourceDir', () => {
    const result = loadConfig({ ...VALID_CONFIG, sourceDir: 'coniva/' });
    expect(result.ok).toBeTrue();
    if (result.ok) {
      expect(result.config.sourceDir).toBe('coniva');
    }
  ...
  test('strips trailing slash from cacheDir', () => {
    const input = {
      ...VALID_CONFIG,
      hostTargets: [
        { kind: 'react-app' as const, mode: 'live' as const, runtimeMode: 'cache-dir' as const, cacheDir: '.coniva/cache/' },
      ],
    };
    ...
    if (result.ok) {
      ...
      expect(target.cacheDir).toBe('.coniva/cache');
    }
  });
  ...
  test('includes configPath in error messages when provided', () => {
    const result = loadConfig(null, 'packages/my-app/coniva.config.ts');
    expect(result.ok).toBeFalse();
    if (!result.ok) {
      expect(result.error).toContain('packages/my-app/coniva.config.ts');
    }
  });
});
