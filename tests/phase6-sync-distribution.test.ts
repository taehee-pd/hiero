...
describe('6.1 — SyncTarget schema', () => {
  test('SyncTarget type for local-directory', () => {
    const target: SyncTarget = {
      ...
      adapterConfig: { runtimePackage: '@coniva/runtime-react', outputDir: 'src' },
    };
...
describe('6.4 — Adapter PR sync types', () => {
  ...
  test('AdapterPrSyncRequest with adapter config', () => {
    ...
    const request: AdapterPrSyncRequest = {
      ...
      actor: { name: 'User' },
      adapterConfig: {
        runtimePackage: '@my-scope/coniva-runtime',
        outputDir: 'lib/icons',
      },
    };
    expect(request.adapterConfig?.runtimePackage).toBe('@my-scope/coniva-runtime');
  });
});
