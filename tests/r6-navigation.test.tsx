 ...
 * - UX-2.5 — Global `?` key dispatches `hiero:open-shortcuts`.
 *
...
describe('R6 · UX-2.5 — `?` key opens the shortcuts cheat sheet', () => {
  ...
  test('`?` keydown fires a `hiero:open-shortcuts` CustomEvent', () => {
    let fired = false;
    const listener = () => {
      fired = true;
    };
    ...
    g.addEventListener!('hiero:open-shortcuts', listener as EventListener);
    try {
      handleEditorKeyDown({
        key: '?',
        target: { tagName: 'DIV' },
      ...
      g.removeEventListener!('hiero:open-shortcuts', listener as EventListener);
    }
  });

  test('Shift+/ also fires `hiero:open-shortcuts` (US layout fallback)', () => {
    let fired = false;
    const listener = () => {
      fired = true;
    };
