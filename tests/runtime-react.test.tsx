...
import { HieroIcon, useIconState } from '../lib/runtime-react';
...
describe('HieroIcon', () => {
  ...
  test('renders with custom size override', () => {
    const html = renderToStaticMarkup(<HieroIcon icon={makeTestIcon()} size={48} />);
    expect(html).toContain('width:48px');
    expect(html).toContain('height:48px');
  });

  test('renders with color prop', () => {
    const html = renderToStaticMarkup(<HieroIcon icon={makeTestIcon()} color="#ff0000" />);
    expect(html).toContain('color:#ff0000');
  });

  test('renders with className', () => {
    const html = renderToStaticMarkup(<HieroIcon icon={makeTestIcon()} className="icon-cls" />);
    expect(html).toContain('class="icon-cls"');
  });

  test('resolves variant by size number', () => {
    const html = renderToStaticMarkup(<HieroIcon icon={makeTestIcon()} variant={16} />);
    expect(html).toContain('width:16px');
    expect(html).toContain('height:16px');
  });

  test('resolves variant by id string', () => {
    const html = renderToStaticMarkup(<HieroIcon icon={makeTestIcon()} variant="v16" />);
    expect(html).toContain('width:16px');
  });

  test('falls back to first variant for unknown variant', () => {
    const html = renderToStaticMarkup(<HieroIcon icon={makeTestIcon()} variant="v999" />);
  ...
  test('renders with custom style', () => {
    const html = renderToStaticMarkup(
      <HieroIcon icon={makeTestIcon()} style={{ opacity: 0.5, borderRadius: 4 }} />,
    );
  ...
  test('labeled icon renders role="img" and aria-label', () => {
    const html = renderToStaticMarkup(<HieroIcon icon={makeTestIcon()} label="Right arrow" />);
