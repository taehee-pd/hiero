...
describe('svg import flow', () => {
  test('imports an svg file into the current project and activates the new icon', async () => {
    ...
    expect(Object.keys(importedIcon?.states.default.layers ?? {})).toEqual([
      'frame',
      'slash',
    ]);
    expect(importedIcon?.states.default.layers.frame).toMatchObject({
      id: 'frame',
      path: {
    ...
    expect(importedIcon?.states.default.layers.slash).toMatchObject({
      id: 'slash',
      path: {
        d: 'M4 18 L18 26',
      },
