...
describe('svg import flow', () => {
  test('imports an svg file into the current project and activates the new icon', async () => {
    ...
    expect(state.currentTypeId).toBe('default');
    expect(importedIcon?.name).toBe('Media Toggle');
    expect(importedIcon?.variants.v32.viewBox).toEqual([0, 0, 32, 32]);
    expect(Object.keys(importedIcon?.variants.v32.types!.default.layers ?? {})).toEqual([
      'frame',
      'slash',
    ]);
    expect(importedIcon?.variants.v32.types!.default.layers.frame).toMatchObject({
      id: 'frame',
      path: {
    ...
    expect(importedIcon?.variants.v32.types!.default.layers.slash).toMatchObject({
      id: 'slash',
      path: {
        d: 'M4 18 L18 26',
      },
