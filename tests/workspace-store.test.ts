...
describe('workspace store', () => {
  ...
  test('switches icon sets and editor tabs', () => {
    ...
    editorStore.getState().insertIcon(structuredClone(SAMPLE_PROJECT.icons['icon-play']));

    const firstTabId = editorStore.getState().openIconTab(originalSetId, 'icon-chevron');
    const secondTabId = editorStore.getState().openIconTab(nextSetId, 'icon-play');
    ...
    expect(editorStore.getState().currentIconId).toBe('icon-play');
    ...
    expect(editorStore.getState().currentIconId).toBe('icon-chevron');
  });
});
